...
public class CarouselLayoutCalculator
{
    ...
    public CarouselLayoutCalculator(RectTransform[] buttons, Vector2[] originalPositions, float centerScale, float sideScale, float distanceFromCenter)
    {
        _buttons = buttons;
    ...
    private float GetAccumulatedOffsetX(int buttonIndex, int centerIndex)
    {
        ...
        {
            positionX += direction * GetStepDistance(stepIndex, stepIndex + (int)direction, centerIndex);
        }
