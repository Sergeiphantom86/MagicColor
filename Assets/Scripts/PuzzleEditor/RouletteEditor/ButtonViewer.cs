...
namespace PuzzleEditor.RouletteEditor
{
    public class ButtonViewer : MonoBehaviour
    {
        ...
        private void Move()
        {
            _sequence.Join(_transform.DOMove(_targetPosition, _duration).SetEase(Ease.Linear));
        }

        private void Scale()
        {
            _sequence.Join(_transform.DOScale(_endScale, _duration).SetEase(Ease.OutBack));
        }
    }
}